...
class Siminstance:
    ...
    def __init__(self, sim_path, workflow_config, disabledLoggers=False):
        ...
        self.logger.debug("Path to simulation instance source: " + str(self.path))
        self.logger.debug("UID of sim instance: {}".format(self.uid))
        self.logger.debug("Local logs path: {}".format(self.local_logs))
        ...
        self.logger.debug("Local results path: {}".format(self.local_results_path))

        self.local_exec = self.workflow_config["local_sim_exec"]
        self.local_exec_path = os.path.join(self.path, self.local_exec)
        self.logger.debug("Local executable name: {}".format(self.local_exec))
        ...
        self.logger.debug("Local config path: {}".format(self.local_config_path))

        self.local_out = self.workflow_config["local_sim_out"]
        self.local_out_path = os.path.join(self.path, self.local_out)
        self.logger.debug("Local out path: {}".format(self.local_out_path))

        self.logger.debug("Reading in config file: " + str(self.local_config_path))
    ...
    def __create_omnet_make_make_command(self):
        ...
        if (self.local_exec):
            self.logger.debug("opp_makemake will use {} as executable filename".format(self.local_exec))
            make_make_command += ["-o", self.local_exec]

        if (self.cnf.tryGet("omnet", "make")):
            self.logger.debug("Omnet config has specification for make")

            if (self.cnf.tryGet("omnet", "make", "opp_makemake")):
                self.logger.debug("Omnet config has specification for opp_makemake")

                if (self.cnf.tryGet("omnet", "make", "opp_makemake", "force")):
                    self.logger.debug("opp_makemake will force overwrite of makefile")
                    make_make_command += ["-f"]

                # TODO: make sure path exists
                if (self.local_out_path):
                    self.logger.debug("opp_makemake will output executable folder {}".format(self.local_out_path))
                    make_make_command += ["-O", self.local_out_path]
        else:
            self.logger.warn("Omnet config has no specification for opp_makemake")
            self.logger.debug("Defaulting to: 'opp_makemake -f'")
            make_make_command += ["-f"]
    ...
    def __create_omnet_make_command(self):
        ...
        if (self.cnf.tryGet("omnet", "make")):
            ...
            if (self.cnf.tryGet("omnet", "make", "verbose")):
                self.logger.debug("Make output will be verbose")
                make_command += ["V=1"]
            if (self.cnf.tryGet("omnet", "make", "ignoreWarnings")):
                self.logger.debug("Adding following ignore warnings to make command: " + ", ".join(self.cnf.tryGet("omnet", "make", "ignoreWarnings")))
            ...
            self.logger.debug("Defaulting to: 'make'")

        self.logger.debug("Make command:\n"+ " ".join(make_command))
        return make_command
    ...
    def __create_omnet_sim_command(self):
        ...
        if (self.cnf.tryGet("omnet", "simulation", "param_sweep", "active")):
            self.logger.debug("Omnet execution will make use of param_sweep")
            # TODO: integrate usage of param_sweep functionality
        else:
            self.logger.debug("Omnet execution will not make use of param_sweep (i.e., it will be sequential)")

        if (self.local_exec):
            simulation_command += ["./" + self.local_exec]
            self.logger.debug("Omnet executable is: {}".format(self.local_exec))
        else:
        ...
        if (self.cnf.tryGet("omnet", "simulation", "environment")):
            ...
            self.logger.debug("Omnet environment is: {}".format(self.cnf.tryGet("omnet", "simulation", "environment")))
        else:
        ...
        if (self.local_results_path):
            ...
            self.logger.debug("Omnet result output path is: {}".format(self.local_results_path))
        else:
        ...
        if (self.cnf.tryGet("omnet", "simulation", "time_limit")):
            ...
            if (time_limit_type == "real-time"):
                self.logger.debug("Omnet will run with time limit type {} for duration of {}".format(time_limit_type, time_limit_duration))
        ...
        if (self.cnf.tryGet("omnet", "simulation", "record_eventlog")):
            self.logger.debug("Omnet will record eventlog")
        ...
        #     self.logger.debug("Omnet will oversubscribe")
        ...
        if (self.cnf.tryGet("omnet", "simulation", "pdes")):
            ...
            self.logger.debug("Omnet execution will perform pdes with {} lps".format(num_lps))
            ...
            self.logger.debug("Omnet execution will not make use of pdes (i.e., it will be sequential)")



        if (self.cnf.tryGet("omnet", "simulation", "config")):
            simulation_command += ["-c", self.cnf.tryGet("omnet", "simulation", "config")]
            self.logger.debug("Omnet config is: {}".format(self.cnf.tryGet("omnet", "simulation", "config")))
        else:
    ...
    def compile(self):
        ...
        if self.cnf.tryGet("omnet"):
            if self.cnf.tryGet("compile"):
                self.logger.debug("Omnet config data present in config file")
                ...
                self.logger.debug("Running opp_makemake command")
                ...
                self.logger.debug("Opp_makemake execution return code: {}".format(make_make_output.returncode))

                if (make_make_output.returncode == 0):
                    self.logger.debug("Opp_makemake execution was successfull")
                else:
                ...
                self.logger.debug("Running make command: {}".format(make_command))
                ...
                self.logger.debug("Make execution return code: {}".format(make_output.returncode))

                if (make_output.returncode == 0):
                    self.logger.debug("Make execution was successfull")
                else:
        ...
        self.logger.debug("Compilation took {:.4} seconds".format(self.stats.get_stat("compilation", "compile_time")))
    ...
    def run(self):
        ...
        if (self.cnf.tryGet("omnet")):
            ...
            if (self.cnf.tryGet("omnet", "simulation")):
                self.logger.debug("Omnet config has specification for simulation execution")
            ...
            self.logger.debug("Running simulation command: {}".format(simulation_command))
            ...
            self.logger.debug("Simulation execution return code: {}".format(simulation_output.returncode))

            if (simulation_output.returncode == 0):
                self.logger.debug("Simulation execution was successfull")
            else:
        ...
        self.logger.debug("Simulation took {:.4} seconds".format(self.stats.get_stat("simulation", "simulation_time")))
        ...
        if self.transform_scalar_output:
            self.logger.debug("Transforming scalar output to CSV-R format")
