...
@replicated_to_parent
@replicate('num_agents')
@replicate('num_agents_avail')
class Hyperheuristic:
    ...
    def __getstate__(self):
        ...
        attributes, particularly anything related to multiprocessing executors or
        threads that should not be transferred.
        """
        state = self.__dict__.copy()
        # The logger is not pickleable and should be re-initialized in the new process if needed.
        state['logger'] = None
        # The rpc_context holds unpickleable Queue objects and is not needed by the shadow copy.
    ...
    def __setstate__(self, state):
        ...
        The logger and other excluded attributes will be None in the shadow copy,
    ...
    def __init__(self, heuristic_space='default.txt', problems=None, parameters=None, file_label='', weights_array=None, pass_finalised_positions=False, file_details=None, heur_coordinator=None, search_operator_space_name = None, updateMHProgress = None, rpc_context=None):
        ...
        if problems:
            ...
            self.heur_coordinator = heur_coordinator # This is now the RPC Proxy
            ...
            raise HyperheuristicError('Problems or a heur_coordinator (proxy) and search_operator_space_name must be provided.')
    ...
    def _check_finalisation(self, step, stag_counter, *args):
        ...
        if self.heur_coordinator is not None:
            ...
            if finalize:
                self.heur_coordinator.hh_base.hh_disable_hh_and_distribute_Resources(
                    self.search_operator_space_name, 
                    step, 
                    finalize_reason
                )
    ...
    def _solve_static(self, save_steps=True):
        ...
        if self.parameters['verbose']:
            self.logger.info('{} time: {} :: Step: {:4d}, Action: {:12s}, Temp: {:.2e}, Card: {:3d}, Perf: {:.2e} [Initial]'.format(
...
, step, 'None', temperature, len(current_solution), self.current_performance))
        ...
        while not self._check_finalisation(step, stag_counter,
            ...
            # Pull for updates from the main process
            if self.heur_coordinator is not None:
                pending_updates = self.heur_coordinator.hh_base.get_pending_updates(self.search_operator_space_name)
                if pending_updates:
                    # Process any relevant updates from the dictionary
                    if 'num_agents_avail' in pending_updates:
                        self.give_avail_agents_for_next_step(pending_updates['num_agents_avail'])
                    # Can add more key checks here in the future
            ...
            verbose_log =""
            if self.parameters['verbose']:
                verbose_log += '{} time: {} :: Step: {:4d}, Action: {:12s}, Temp: {:.2e}, Card: {:3d}, '.format(
                    self.file_label, (end_time - start_time), step, action, temperature, len(candidate_solution)) + \
                      'candPerf: {:.2e}, currPerf: {:.2e}, bestPerf: {:.2e}'.format(
                          candidate_performance, self.current_performance, self.best_performance)
...
# Todo add threaded replica evaluation
    def _solve_neural_network(self, save_steps=True):
        ...
        for rep in range(self.parameters['num_replicas']):
            ...
            while not self._check_finalisation(step, stag_counter):
                ...
                # Print update
                if self.parameters['verbose']:
                    verbose_log += '{} : time: {}:: Neural Network, Rep: {:3d}, Step: {:3d}, Trial: {:3d}, SO: {:30s}, currPerf: {:.2e}, candPerf: {:.2e}, ' + \
                        'csl: {:3d}'.format(
                            self.file_label, (end_time - start_time), rep + 1, step + 1, stag_counter,
                                             candidate_search_operator[0][0] + ' & ' + candidate_search_operator[0][2][
                                                                                       :4],
                            best_fitness[-1], current_fitness, len(self.current_space))
    ...
    def evaluate_candidate_solution(self, encoded_sequence, collection_finalised_positions_previous_step=None):
        """
        ...
        collection_finalised_positions_previous_step (optional):
            NOTE: CUSTOM CHANGE BY LARS - Collection of the finalised positions of the previous step for every replica. This is used to pass the finalised positions of the previous step to the next step.
        ...
        # NOTE: CUSTOM CHANGE BY LARS - REPLACED THREADING WITH LOCAL PARALLELIZATION.
        # Run the metaheuristic several times.
        mhs = {}
        mh_threads = []
        mh_thread_results = [None] * self.parameters['num_replicas']
        ...
        for i in range(self.parameters['num_replicas']):
            ...
            # mh_threads.append(threading.Thread(
            #     target=mhs[i].run, 
            #     args=(self.hh_step, self.file_label)
            # ))

            thread = threading.Thread(
                target=lambda res_list, idx, hh_step, file_label: res_list.__setitem__(idx, mhs[i].run(hh_step, file_label)),
                args=(mh_thread_results, i, self.hh_step, self.file_label)
            )
            mh_threads.append(thread)
            thread.start()

        self.logger.debug(f"Running {self.parameters['num_replicas']} metaheuristics in parallel.")

        # Wait for all MH threads for this Hyperheuristic to complete
        for i, thread in enumerate(mh_threads):
            thread.join()
