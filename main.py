...
def create_problem_instance(run, heur_sim_coordinator):
    """
...
def quick_and_dirty_save_hh_positions_to_xlsx():
    ...
    # Initialize a dictionary to store the rescaled positions
    rescaled_positions_data = []

    # Loop through the crossover, mutation, and swarm positions
    for method, positions in positions_data.items():
        for category, position_list in positions.items():
            rescaled_positions = quick_and_dirty_rescale_back_positions(position_list, centre_boundaries, span_boundaries)
            rescaled_positions_data.append([method, category] + rescaled_positions)

    # Convert to a DataFrame
    columns = ['Operator', 'Type'] + [f'Position_{i+1}' for i in range(49)]
    rescaled_positions_df = pd.DataFrame(rescaled_positions_data, columns=columns)

    # Write the DataFrame to an Excel file
    excel_filename = 'rescaled_positions.xlsx'
    rescaled_positions_df.to_excel(excel_filename, index=False)
